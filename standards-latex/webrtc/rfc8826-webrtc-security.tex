...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{RFC 8826}
\fancyhead[R]{Security Considerations for WebRTC}
\fancyfoot[C]{\thepage}

\title{Security Considerations for WebRTC}
\author{E. Rescorla \\
Mozilla}
\date{January 2021 \\ RFC 8826 \\ Category: Standards Track \\ ISSN: 2070-1721}
...
Each of their browsers exposes standardized JavaScript (JS) calling APIs
(implemented as browser built-ins) which are used by the Web server to
set up a call between Alice and Bob.  The Web server also serves as the
signaling channel to transport control messages between the browsers.
...
acting as softphones), control has moved to the central Web server; the
browser simply provides API points that are used by the calling service.
As with any Web application, the Web server can move logic between the
server and JavaScript in the browser, but regardless of where the code
is executing, it is ultimately under control of the server.
...
\section{The Browser Threat Model}
...
\subsection{Access to Local Resources}
...
same \emph{origin} from whence the script came [RFC6454] they are not able
...
\subsection{Bypassing SOP: CORS, WebSockets, and Consent to Communicate}
...
when a script from origin A executes a potentially unsafe cross-origin
request, the browser instead contacts the target server to
...
transactions.  As with CORS, a WebSockets transaction starts
...
attacking script generates traffic which is acceptable to some non-Web
protocol state machine.  In order to resist this form of attack,
...
\section{Security for WebRTC Applications}

\subsection{Access to Local Devices}
...
\begin{enumerate}
\item You are sending your media to entity A because you want to talk
      to entity A (e.g., your mother).

\item Entity A (e.g., a calling service) asks to access the user's
...
\subsubsection{Threats from Screen Sharing}
...
(See \url{https://lists.w3.org/Archives/Public/public-webrtc/2013Mar/0024.html}
for a full analysis.)
...
\subsubsection{Calling Scenarios and User Expectations}
...
\paragraph{Dedicated Calling Services}
...
\paragraph{Calling the Site You're On}
...
\subsubsection{Origin-Based Security}
...
\begin{description}
\item[Individual Consent]
   Ask the user for permission for each call.

\item[Callee-oriented Consent]
   Only allow calls to a given user.

\item[Cryptographic Consent]
...
\subsubsection{Security Properties of the Calling Page}
...
\texttt{http://calling-service.example.com}.  If I ever use my computer on an
...
\begin{enumerate}
\item I connect to \texttt{http://anything.example.org/}.  Note that this site
      is unaffiliated with the calling service.

\item The attacker modifies my HTTP connection to inject an IFRAME (or
      a redirect) to \texttt{http://calling-service.example.com}.

\item The attacker forges the response from
      \texttt{http://calling-service.example.com/} to inject JS to initiate a
      call to themselves.
\end{enumerate}
...
parent page is safe.  Note: This issue is not restricted to \emph{pages}
...
attacker to infect the browser's notion of that origin semi-permanently.

\subsection{Communications Consent Verification}
...
but conveniently we already need one in order to do NAT hole-punching.
...
application, but would need to also be known and used by the STUN-receiving
...
\begin{itemize}
\item STUN checks without using ICE (i.e., the non-RTC-web endpoint sets
      up a STUN responder).

\item Use of the RTP Control Protocol (RTCP) as an implicit reachability
      check.
\end{itemize}
...
address reveals a lot of information about the callee's
...
(\url{https://www.torproject.org}) or a VPN.  However, because sites can
...
\begin{description}
\item[Retrospective compromise of calling service:]
...
\item[During-call attack by calling service:]
...
\subsubsection{Protecting Against Retrospective Compromise}
...
\subsubsection{Protecting Against During-Call Attack}
...
\paragraph{Short Authentication Strings}
...
\paragraph{Third-Party Identity}
...
\paragraph{Page Access to Media}
...
\subsubsection{Correlation of Anonymous Calls}
...
(if generated according to [RFC6222] rather than the privacy-preserving
mode of [RFC7022]).  In order to prevent this type of
...
\begin{description}[style=nextline, labelwidth=10em, leftmargin=10em]
...
\begin{description}[style=nextline, labelwidth=10em, leftmargin=10em]
\item[{[abarth-rtcweb]}] Barth, A., ``Prompting the user is security failure'', RTC-Web
   Workshop, September 2010,
   \url{http://rtc-web.alvestrand.com/home/papers/barth-security-prompt.pdf?attredirects=0}.

\item[{[cranor-wolf]}] Sunshine, J., Egelman, S., Almuhimedi, H., Atri, N., and
   ...
   \url{https://www.usenix.org/legacy/event/sec09/tech/full_papers/sunshine.pdf}.

\item[{[deepfakes-fraud]}] Statt, N., ``Thieves are now using AI deepfakes to trick
   companies into sending them money'', September 2019,
   \url{https://www.theverge.com/2019/9/5/20851248/deepfakes-ai-fake-audio-phone-calls-thieves-trick-companies-stealing-money}.

\item[{[deepfakes-ftc]}] Lyons, K., ``FTC says the tech behind audio deepfakes is
   getting better'', January 2020,
   \url{https://www.theverge.com/2020/1/29/21080553/ftc-deepfakes-audio-cloning-joe-rogan-phone-scams}.

\item[{[fetch]}] van Kesteren, A., ``Fetch'',
   \url{https://fetch.spec.whatwg.org/}.

\item[{[finer-grained]}] Jackson, C. and A. Barth, ``Beware of Finer-Grained
   Origins'', Web 2.0 Security and Privacy (W2SP 2008), July
   2008.

\item[{[Fingerprinting]}] Doty, N., Ed., ``Mitigating Browser Fingerprinting in Web
   Specifications'', March 2019,
   \url{https://www.w3.org/TR/fingerprinting-guidance/}.

\item[{[huang-w2sp]}] Huang, L-S., Chen, E.Y., Barth, A., Rescorla, E., and C.
   Jackson, ``Talking to Yourself for Fun and Profit'', Web 2.0
   Security and Privacy (W2SP 2011), May 2011.

\item[{[OpenID]}] Sakimura, N., Bradley, J., Jones, M., de Medeiros, B., and
...
\item[{[RFC3552]}] Rescorla, E. and B. Korver, ``Guidelines for Writing RFC
...
\item[{[RFC3760]}] Gustafson, D., Just, M., and M. Nystrom, ``Securely
   Available Credentials (SACRED) - Credential Server
   Framework'', RFC 3760, DOI 10.17487/RFC3760, April 2004,
   \url{https://www.rfc-editor.org/info/rfc3760}.

\item[{[RFC4251]}] Ylonen, T. and C. Lonvick, Ed., ``The Secure Shell (SSH)
   Protocol Architecture'', RFC 4251, DOI 10.17487/RFC4251,
   January 2006, \url{https://www.rfc-editor.org/info/rfc4251}.
...
\item[{[RFC5479]}] Wing, D., Ed., Fries, S., Tschofenig, H., and F. Audet,
   ``Requirements and Analysis of Media Security Management
   Protocols'', RFC 5479, DOI 10.17487/RFC5479, April 2009,
   \url{https://www.rfc-editor.org/info/rfc5479}.
...
\item[{[RFC6189]}] Zimmermann, P., Johnston, A., Ed., and J. Callas, ``ZRTP:
   Media Path Key Agreement for Unicast Secure RTP'',
   RFC 6189, DOI 10.17487/RFC6189, April 2011,
   \url{https://www.rfc-editor.org/info/rfc6189}.

\item[{[RFC6222]}] Begen, A., Perkins, C., and D. Wing, ``Guidelines for
   Choosing RTP Control Protocol (RTCP) Canonical Names
   (CNAMEs)'', RFC 6222, DOI 10.17487/RFC6222, April 2011,
   \url{https://www.rfc-editor.org/info/rfc6222}.
...
\item[{[RFC6749]}] Hardt, D., Ed., ``The OAuth 2.0 Authorization Framework'',
...
\item[{[RFC7033]}] Jones, P., Salgueiro, G., Jones, M., and J. Smarr,
   ``WebFinger'', RFC 7033, DOI 10.17487/RFC7033, September
   2013, \url{https://www.rfc-editor.org/info/rfc7033}.
...
\item[{[RFC8827]}] Rescorla, E., ``WebRTC Security Architecture'', RFC 8827,
...
\item[{[SWF]}] ``SWF File Format Specification Version 19'', April 2013,
   \url{https://www.adobe.com/content/dam/acom/en/devnet/pdf/swf-file-format-spec.pdf}.

\item[{[whitten-johnny]}] Whitten, A. and J.D. Tygar, ``Why Johnny Can't Encrypt: A
   Usability Evaluation of PGP 5.0'', Proceedings of the 8th
   USENIX Security Symposium, August 1999,
   \url{https://www.usenix.org/legacy/publications/library/proceedings/sec99/whitten.html}.
\end{description}
