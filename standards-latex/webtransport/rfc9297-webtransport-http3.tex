...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{RFC 9297}
\fancyhead[R]{HTTP Datagrams and the Capsule Protocol}
\fancyfoot[C]{\thepage}

\title{HTTP Datagrams and the Capsule Protocol\\
\large RFC 9297\\
\large Internet Engineering Task Force (IETF)\\
\large Category: Standards Track\\
\large ISSN: 2070-1721}
...
\item[Quarter Stream ID:]  A variable-length integer that contains the value
   ...
   treated as an HTTP/3 connection error of type H3\_DATAGRAM\_ERROR
   (0x33).

\item[HTTP Datagram Payload:]  The payload of the datagram, whose semantics
...
connection error of type H3\_DATAGRAM\_ERROR (0x33).
...
connection error of type H3\_ID\_ERROR.  Generating an error is not
...
\subsubsection{The SETTINGS\_H3\_DATAGRAM HTTP/3 Setting}

An endpoint can indicate to its peer that it is willing to receive
HTTP/3 Datagrams by sending the SETTINGS\_H3\_DATAGRAM (0x33) setting
with a value of 1.

The value of the SETTINGS\_H3\_DATAGRAM setting MUST be either 0 or 1.
A value of 0 indicates that the implementation is not willing to
receive HTTP Datagrams.  If the SETTINGS\_H3\_DATAGRAM setting is
received with a value that is neither 0 nor 1, the receiver MUST
terminate the connection with error H3\_SETTINGS\_ERROR.

QUIC DATAGRAM frames MUST NOT be sent until the SETTINGS\_H3\_DATAGRAM
setting has been both sent and received with a value of 1.

When clients use 0-RTT, they MAY store the value of the server's
SETTINGS\_H3\_DATAGRAM setting.  Doing so allows the client to send
QUIC DATAGRAM frames in 0-RTT packets.  When servers decide to accept
0-RTT data, they MUST send a SETTINGS\_H3\_DATAGRAM setting greater
...
stores the value of the SETTINGS\_H3\_DATAGRAM setting with their 0-RTT
state, they MUST validate that the new value of the
SETTINGS\_H3\_DATAGRAM setting sent by the server in the handshake is
greater than or equal to the stored value; if not, the client MUST
terminate the connection with error H3\_SETTINGS\_ERROR.  In all cases,
the maximum permitted value of the SETTINGS\_H3\_DATAGRAM setting
parameter is 1.

It is RECOMMENDED that implementations that support receiving HTTP/3
Datagrams always send the SETTINGS\_H3\_DATAGRAM setting with a value
of 1, even if the application does not intend to use HTTP/3
Datagrams.  This helps to avoid ``sticking out''; see Section 4.

\subsection{HTTP Datagrams Using Capsules}
...
\subsection{The Capsule Protocol}
...
\begin{description}[style=unboxed, leftmargin=0.5cm]
\item[Capsule Type:]  A variable-length integer indicating the type of the
   capsule.  An IANA registry is used to manage the assignment of
   Capsule Types; see Section 5.4.

\item[Capsule Length:]  The length, in bytes, of the Capsule Value field,
   which follows this field, encoded as a variable-length integer.
   Note that this field can have a value of zero.

\item[Capsule Value:]  The payload of this Capsule.  Its semantics are
...
\begin{itemize}
\item The Capsule Protocol is not in use unless the response includes a
  2xx (Successful) or 101 (Switching Protocols) status code.

\item When the Capsule Protocol is in use, the associated HTTP request
...
\subsection{The Capsule-Protocol Header Field}
...
\subsection{The DATAGRAM Capsule}
...
sending the HTTP/3 SETTINGS\_H3\_DATAGRAM setting, it ``sticks out''.  In
...
\subsection{HTTP/3 Setting}

IANA has registered the following entry in the ``HTTP/3 Settings''
registry maintained at \url{https://www.iana.org/assignments/http3-parameters}:

\begin{description}[style=unboxed, leftmargin=0.5cm]
\item[Value:]  0x33
\item[Setting Name:]  SETTINGS\_H3\_DATAGRAM
\item[Default:]  0
\item[Status:]  permanent
\item[Reference:]  RFC 9297
\item[Change Controller:]  IETF
\item[Contact:]  HTTP\_WG; HTTP working group; ietf-http-wg@w3.org
\item[Notes:]  None
\end{description}

\subsection{HTTP/3 Error Code}
...
\begin{description}[style=unboxed, leftmargin=0.5cm]
\item[Value:]  0x33
\item[Name:]  H3\_DATAGRAM\_ERROR
\item[Description:]  Datagram or Capsule Protocol parse error
\item[Status:]  permanent
\item[Reference:]  RFC 9297
\item[Change Controller:]  IETF
\item[Contact:]  HTTP\_WG; HTTP working group; ietf-http-wg@w3.org
...
\subsection{HTTP Header Field Name}
...
\begin{description}[style=unboxed, leftmargin=0.5cm]
\item[Field Name:]  Capsule-Protocol
\item[Template:]  None
\item[Status:]  permanent
\item[Reference:]  RFC 9297
\item[Comments:]  None
...
\begin{description}[style=unboxed, leftmargin=0.5cm]
\item[Value:]  0x00
\item[Capsule Type:]  DATAGRAM
\item[Status:]  permanent
\item[Reference:]  RFC 9297
\item[Change Controller:]  IETF
\item[Contact:]  MASQUE Working Group masque@ietf.org
\item[Notes:]  None
\end{description}
...
\begin{description}[style=unboxed, leftmargin=1cm]
\item[{[HTTP]}] Fielding, R., Ed., Nottingham, M., Ed., and J. Reschke,
   Ed., ``HTTP Semantics'', STD 97, RFC 9110,
   DOI 10.17487/RFC9110, June 2022,
   \url{https://www.rfc-editor.org/info/rfc9110}.

\item[{[HTTP/1.1]}] Fielding, R., Ed., Nottingham, M., Ed., and J. Reschke,
   Ed., ``HTTP/1.1'', STD 99, RFC 9112, DOI 10.17487/RFC9112,
   June 2022, \url{https://www.rfc-editor.org/info/rfc9112}.

\item[{[HTTP/2]}] Thomson, M., Ed. and C. Benfield, Ed., ``HTTP/2'', RFC 9113,
   DOI 10.17487/RFC9113, June 2022,
   \url{https://www.rfc-editor.org/info/rfc9113}.

\item[{[HTTP/3]}] Bishop, M., Ed., ``HTTP/3'', RFC 9114, DOI 10.17487/RFC9114,
   June 2022, \url{https://www.rfc-editor.org/info/rfc9114}.

\item[{[IANA-POLICY]}] Cotton, M., Leiba, B., and T. Narten, ``Guidelines for
   Writing an IANA Considerations Section in RFCs'', BCP 26,
   RFC 8126, DOI 10.17487/RFC8126, June 2017,
   \url{https://www.rfc-editor.org/info/rfc8126}.

\item[{[QUIC]}] Iyengar, J., Ed. and M. Thomson, Ed., ``QUIC: A UDP-Based
   Multiplexed and Secure Transport'', RFC 9000,
   DOI 10.17487/RFC9000, May 2021,
   \url{https://www.rfc-editor.org/info/rfc9000}.

\item[{[QUIC-DGRAM]}] Pauly, T., Kinnear, E., and D. Schinazi, ``An Unreliable
...
\item[{[STRUCTURED-FIELDS]}] Nottingham, M. and P-H. Kamp, ``Structured Field Values for
   HTTP'', RFC 8941, DOI 10.17487/RFC8941, February 2021,
   \url{https://www.rfc-editor.org/info/rfc8941}.

\item[{[TCP]}] Eddy, W., Ed., ``Transmission Control Protocol (TCP)'',
...
\begin{description}[style=unboxed, leftmargin=1cm]
\item[{[DPLPMTUD]}] Fairhurst, G., Jones, T., T\"uxen, M., R\"ungeler, I., and T.
   V\"olker, ``Packetization Layer Path MTU Discovery for
   Datagram Transports'', RFC 8899, DOI 10.17487/RFC8899,
   September 2020, \url{https://www.rfc-editor.org/info/rfc8899}.

\item[{[EXT-CONNECT2]}] McManus, P., ``Bootstrapping WebSockets with HTTP/2'',
   RFC 8441, DOI 10.17487/RFC8441, September 2018,
   \url{https://www.rfc-editor.org/info/rfc8441}.

\item[{[EXT-CONNECT3]}] Hamilton, R., ``Bootstrapping WebSockets with HTTP/3'',
   RFC 9220, DOI 10.17487/RFC9220, June 2022,
   \url{https://www.rfc-editor.org/info/rfc9220}.

\item[{[PRIORITY]}] Oku, K. and L. Pardue, ``Extensible Prioritization Scheme
   for HTTP'', RFC 9218, DOI 10.17487/RFC9218, June 2022,
   \url{https://www.rfc-editor.org/info/rfc9218}.

\item[{[WEBSOCKET]}] Fette, I. and A. Melnikov, ``The WebSocket Protocol'',
